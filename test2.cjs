...
const targetMs = new Date('2026-06-15T12:00:00Z').getTime();
const days = (targetMs - J2000) / DAY_MS;

const bodies = ['Sun', 'Mercury', 'Venus', 'Mars', 'Jupiter', 'Saturn', 'Uranus', 'Neptune', 'Pluto'];
const pos = {};
for (const b of bodies) {
  pos[b] = getLong(b, days);
}

console.log("Positions on 2026-06-15:");
for (const b of bodies) {
  console.log(`${b}: ${pos[b].toFixed(2)}`);
}
